...
xqc_fuzz_capsule_decode_only(const uint8_t *data, size_t len)
{
    ...
    xqc_int_t rc =
        xqc_h3_ext_capsule_decode(data, len, &type, &payload, &payload_len, &consumed);
    if (rc == XQC_OK) {
        assert(payload >= data);
...
xqc_fuzz_capsule_roundtrip(const uint8_t *data, size_t len)
{
    ...
    xqc_int_t rc =
        xqc_h3_ext_capsule_encode(out, sizeof(out), &written, type, payload, payload_len);
    if (rc != XQC_OK) {
        return;
    }
    ...
    rc = xqc_h3_ext_capsule_decode(out, written, &decoded_type, &decoded_payload,
...
xqc_fuzz_udp_unframe_only(const uint8_t *data, size_t len)
{
    ...
    xqc_int_t rc = xqc_h3_ext_masque_unframe_udp(data, len, &quarter_id, &context_id,
                                                 &payload, &payload_len);
    if (rc == XQC_OK) {
        assert(payload >= data);
        assert(payload <= data + len);
...
xqc_fuzz_udp_roundtrip(const uint8_t *data, size_t len)
{
    ...
    xqc_int_t rc = xqc_h3_ext_masque_frame_udp(out, sizeof(out), &written, stream_id,
                                               payload, payload_len);
    if (rc != XQC_OK) {
        return;
    }
    ...
    rc = xqc_h3_ext_masque_unframe_udp(out, written, &quarter_id, &context_id,
...
xqc_fuzz_address_assign_parse(const uint8_t *data, size_t len)
{
    ...
    xqc_int_t rc = xqc_h3_ext_connectip_parse_address_assign(
        data, len, &request_id, &ip_version, ip_addr, &ip_addr_len, &prefix_len,
        &consumed);
    ...
    assert((ip_version == 4 && ip_addr_len == 4) ||
           (ip_version == 6 && ip_addr_len == 16));
...
LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    switch (mode) {
    case 0: xqc_fuzz_udp_unframe_only(payload, payload_len); break;
    case 1: xqc_fuzz_udp_roundtrip(payload, payload_len); break;
    case 2: xqc_fuzz_capsule_decode_only(payload, payload_len); break;
    case 3: xqc_fuzz_capsule_roundtrip(payload, payload_len); break;
    case 4: xqc_fuzz_address_assign_parse(payload, payload_len); break;
    case 5:
        xqc_fuzz_route_advertisement_parse(payload, payload_len);
        xqc_fuzz_route_advertisement_validate(payload, payload_len);
        break;
    case 6: xqc_fuzz_ip_packet_validate(payload, payload_len); break;
    case 7:
    default: xqc_fuzz_ipv6_mtu_check(payload, payload_len); break;
    }
