...
#define XQC_TEST_CLIENT_SECRET                         \
    "\x75\xf5\xba\x26\xff\x42\x51\x13\x20\x76\x4e\xd7" \
    "\x36\x5c\x20\x8d\x5d\x9c\x8a\xd1\x01\xe5\x0f\xc1\xc3\xc5\xaa\xfb\xd6\x3b\x56\x4a"

#define XQC_TEST_SERVER_SECRET                         \
    "\x65\x0b\x75\x13\xab\x55\x6c\xab\xa6\xf4\xda\x9e" \
...
static const uint8_t rfc9001_dcid[] = {0x83, 0x94, 0xc8, 0xf0, 0x3e, 0x51, 0x57, 0x08};
...
static const uint8_t rfc9001_client_key[16] = {0x1f, 0x36, 0x96, 0x13, 0xdd, 0x76,
                                               0xd5, 0x46, 0x77, 0x30, 0xef, 0xcb,
                                               0xe3, 0xb1, 0xa2, 0x2d};

/* client AEAD IV (12 bytes) */
static const uint8_t rfc9001_client_iv[12] = {0xfa, 0x04, 0x4b, 0x2f, 0x42, 0xa3,
                                              0xfd, 0x3b, 0x46, 0xfb, 0x25, 0x5c};

/* client header protection key (AES-128-CTR, 16 bytes) */
static const uint8_t rfc9001_client_hp[16] = {0x9f, 0x50, 0x44, 0x9e, 0x04, 0xa0,
                                              0xe8, 0x10, 0x28, 0x3a, 0x1e, 0x99,
                                              0x33, 0xad, 0xed, 0xd2};

/* server AEAD key (AES-128-GCM, 16 bytes) */
static const uint8_t rfc9001_server_key[16] = {0xcf, 0x3a, 0x53, 0x31, 0x65, 0x3c,
                                               0x36, 0x4c, 0x88, 0xf0, 0xf3, 0x79,
                                               0xb6, 0x06, 0x7e, 0x37};

/* server AEAD IV (12 bytes) */
static const uint8_t rfc9001_server_iv[12] = {0x0a, 0xc1, 0x49, 0x3c, 0xa1, 0x90,
                                              0x58, 0x53, 0xb0, 0xbb, 0xa0, 0x3e};

/* server header protection key (AES-128-CTR, 16 bytes) */
static const uint8_t rfc9001_server_hp[16] = {0xc2, 0x06, 0xb8, 0xd9, 0xb9, 0xf0,
                                              0xf3, 0x76, 0x44, 0x43, 0x0b, 0x49,
                                              0x0e, 0xea, 0xa3, 0x14};
...
xqc_test_rfc9001_initial_secret()
{
    ...
    xqc_int_t ret = xqc_hkdf_extract(initial_secret, INITIAL_SECRET_MAX_LEN, rfc9001_dcid,
                                     sizeof(rfc9001_dcid), salt, saltlen, &md);
    CU_ASSERT(ret == XQC_OK);
...
xqc_test_rfc9001_derive_initial_secrets()
{
    ...
    xqc_int_t ret =
        xqc_crypto_derive_initial_secret(cli_secret, INITIAL_SECRET_MAX_LEN, svr_secret,
                                         INITIAL_SECRET_MAX_LEN, &dcid, salt, saltlen);
    CU_ASSERT(ret == XQC_OK);
...
xqc_test_rfc9001_client_initial_keys()
{
    ...
    xqc_int_t ret = xqc_crypto_derive_keys(crypto, rfc9001_client_initial_secret,
    ...
    CU_ASSERT(memcmp(ckm->key.base, rfc9001_client_key, sizeof(rfc9001_client_key)) == 0);

    CU_ASSERT(ckm->iv.len == sizeof(rfc9001_client_iv));
    CU_ASSERT(memcmp(ckm->iv.base, rfc9001_client_iv, sizeof(rfc9001_client_iv)) == 0);
...
xqc_test_rfc9001_server_initial_keys()
{
    ...
    xqc_int_t ret = xqc_crypto_derive_keys(crypto, rfc9001_server_initial_secret,
                                           INITIAL_SECRET_MAX_LEN, XQC_KEY_TYPE_RX_READ);
    CU_ASSERT(ret == XQC_OK);
    ...
    CU_ASSERT(memcmp(ckm->key.base, rfc9001_server_key, sizeof(rfc9001_server_key)) == 0);

    CU_ASSERT(ckm->iv.len == sizeof(rfc9001_server_iv));
    CU_ASSERT(memcmp(ckm->iv.base, rfc9001_server_iv, sizeof(rfc9001_server_iv)) == 0);
