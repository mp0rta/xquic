...
static xqc_int_t
xqc_test_parse_stream_frame_inner(unsigned char *frame_buf, size_t frame_buf_len,
...
xqc_test_parse_padding_frame()
{
    ...
    CU_ASSERT(ret == XQC_OK);
    CU_ASSERT(pi_padding_mix.pi_frame_types ==
              (XQC_FRAME_BIT_PADDING | XQC_FRAME_BIT_MAX_DATA));
...
xqc_test_stream_frame_offset_overflow()
{
    ...
    ret = xqc_test_parse_stream_frame_inner(frame_overflow, sizeof(frame_overflow),
    ...
    unsigned char frame_boundary[] = {
        ...
        0xFF, 0xFF, 0xFF, 0xFF, 0xFE, /* offset = (1<<62)-2 */
        0x01,                         /* length = 1 */
        0x00                          /* 1 byte data */
    };
    ret = xqc_test_parse_stream_frame_inner(frame_boundary, sizeof(frame_boundary),
                                            &frame, &stream_id, &conn);
    CU_ASSERT(ret == XQC_OK);
    ...
    unsigned char frame_zero_len[] = {
        0x0e, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, /* length = 0 */
    };
    ret = xqc_test_parse_stream_frame_inner(frame_zero_len, sizeof(frame_zero_len),
                                            &frame, &stream_id, &conn);
    CU_ASSERT(ret == XQC_OK);
    CU_ASSERT(stream_id == 0);
    ...
    unsigned char frame_implicit_boundary[] = {
        0x0c,                                                /* STREAM + OFF */
        0x00,                                                /* stream_id = 0 */
        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFE, 0x00 /* 1 byte data */
    };
    ret = xqc_test_parse_stream_frame_inner(frame_implicit_boundary,
                                            sizeof(frame_implicit_boundary), &frame,
                                            &stream_id, &conn);
    CU_ASSERT(ret == XQC_OK);
    CU_ASSERT(stream_id == 0);
    CU_ASSERT(frame.data_offset == ((UINT64_C(1) << 62) - 2));
    ...
    unsigned char frame_implicit_overflow[] = {
        0x0c,                                                /* STREAM + OFF */
        0x00,                                                /* stream_id = 0 */
        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00 /* 1 byte data */
    };
    ret = xqc_test_parse_stream_frame_inner(frame_implicit_overflow,
                                            sizeof(frame_implicit_overflow), &frame,
                                            &stream_id, &conn);
    CU_ASSERT(ret == -XQC_EILLEGAL_FRAME);
    CU_ASSERT(conn->conn_err == TRA_FRAME_ENCODING_ERROR);
    xqc_engine_destroy(conn->engine);
