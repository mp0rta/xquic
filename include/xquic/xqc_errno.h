...
typedef enum {
    ...
    TRA_0RTT_TRANS_PARAMS_ERROR =
        0xE, /**< MUST delete the current saved 0RTT transport parameters */
...
typedef enum {
    ...
    XQC_EANTI_AMPLIFICATION_LIMIT =
        633,                        /**< server reached the anti-amplification limit */
...
typedef enum {
    ...
    XQC_H3_INVALID_STREAM =
        814, /**< invalid stream, such as multiple control streams, etc. */
    XQC_H3_CLOSE_CRITICAL_STREAM =
        815, /**< illegal closure of control stream and qpack encoder/decoder stream */
    XQC_H3_STATE_ERROR = 816, /**< http3 decoding status error */
    XQC_H3_CONTROL_ERROR =
        817, /**< control stream error, such as setting not send first or send twice */
    XQC_H3_CONTROL_DECODE_ERROR = 818, /**< control stream decode error, such as
                                          encountering an unrecognized frame type */
    XQC_H3_CONTROL_DECODE_INVALID =
        819, /**< control stream decode invalid, eg. illegal remaining length */
    ...
    XQC_H3_INVALID_HEADER =
        823, /**< invalid header field, such as the length exceeds the limit, etc. */
    ...
    XQC_H3_BYTESTREAM_FIN_SENT =
        831, /**< try to send data on a bytestream that already sent FIN */
    XQC_H3_BYTESTREAM_MSG_BUF_EXIST =
        832, /**< try to create a msg buf while it already exists */
    XQC_H3_CONTROL_FRAME_UNEXPECTED =
        833, /**< request-only frame received on control stream (RFC 9114 §7.2.1/§7.2.5) */
    XQC_H3_MISSING_SETTINGS =
        834, /**< first frame on control stream is not SETTINGS, RFC 9114 §6.2.1 */
    XQC_H3_REQUEST_FRAME_UNEXPECTED =
        835, /**< control-only frame received on request stream (RFC 9114 §7.2) */
